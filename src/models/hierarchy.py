"""
Hierarchical Multitask Learning Framework for Clinical Text and Structured Data

This script defines a hierarchical multitask model that processes features through
multiple levels of abstraction:
- Level 1: Raw feature processing (low-level patterns)
- Level 2: Intermediate feature abstraction (mid-level patterns)  
- Level 3: High-level semantic representation
- Integration layer: Combines all levels for final predictions

Architecture uses concat projection + MLP with minimal text input.
"""
...
class HierarchicalFeatureEncoder(nn.Module):
    """
    Hierarchical feature encoder that processes features at multiple levels of abstraction
    """
    def __init__(self, num_features=44, base_dim=512):
        super().__init__()
        
        # Level 1: Low-level feature processing (local patterns)
        self.level1_encoder = nn.Sequential(
            nn.BatchNorm1d(num_features),
            GaussianNoise(0.0, 0.1),
            nn.Linear(num_features, base_dim // 2),
            nn.ReLU(),
            nn.BatchNorm1d(base_dim // 2),
            nn.Dropout(0.1)
        )
        
        # Level 2: Mid-level feature processing (regional patterns)
        ...
        # Level 3: High-level feature processing (global patterns)
        ...
        # Integration layer: Combines all levels
        # Input: level1 (256) + level2 (512) + level3 (512) = 1280
        total_features = (base_dim // 2) + base_dim + base_dim
        self.integration_layer = nn.Sequential(
            nn.Linear(total_features, base_dim),
            nn.ReLU(),
            nn.BatchNorm1d(base_dim),
            nn.Dropout(0.1),
            nn.Linear(base_dim, base_dim)
        ...
        # Store dimensions for access
        self.level1_dim = base_dim // 2
        self.level2_dim = base_dim
        self.level3_dim = base_dim
        self.integrated_dim = base_dim
        
    def forward(self, x):
        # Process through each level
        level1_features = self.level1_encoder(x)
        level2_features = self.level2_encoder(level1_features)
        level3_features = self.level3_encoder(level2_features)
        
        # Concatenate all levels for integration
        combined_features = torch.cat([level1_features, level2_features, level3_features], dim=1)
...
class HierarchicalClassifier(nn.Module):
    ...
    """
    def __init__(self, feature_dims, num_classes=6):
        super().__init__()
        
        # Classifiers for each level
        self.level1_classifier = nn.Sequential(
            nn.Linear(feature_dims['level1'], 128),
            nn.ReLU(),
            nn.Dropout(0.2),
        ...
        self.level2_classifier = nn.Sequential(
            nn.Linear(feature_dims['level2'], 256),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(256, 128),
            nn.ReLU(),
        ...
        self.level3_classifier = nn.Sequential(
            nn.Linear(feature_dims['level3'], 512),
            nn.ReLU(),
            nn.Dropout(0.25),
            nn.Linear(512, 256),
            nn.ReLU(),
        ...
        self.integrated_classifier = nn.Sequential(
            nn.Linear(feature_dims['integrated'], 512),
            nn.ReLU(),
            nn.BatchNorm1d(512),
            nn.Dropout(0.3),
        ...
        # Attention mechanism to combine predictions
        self.prediction_attention = nn.Sequential(
            nn.Linear(num_classes * 4, num_classes * 2),  # 4 predictions -> 2x classes
            nn.ReLU(),
            nn.Linear(num_classes * 2, 4),  # 4 attention weights
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, hierarchical_features):  # FIXED: Added 'def' keyword
        ...
        level3_pred = self.level3_classifier(hierarchical_features['level3'])
        integrated_pred = self.integrated_classifier(hierarchical_features['integrated'])
        
        # Combine predictions with attention
        all_predictions = torch.cat([level1_pred, level2_pred, level3_pred, integrated_pred], dim=1)
        attention_weights = self.prediction_attention(all_predictions)
        
        # Weighted combination of predictions
        final_prediction = (
            attention_weights[:, 0:1] * level1_pred +
            attention_weights[:, 1:2] * level2_pred +
            attention_weights[:, 2:3] * level3_pred +
            attention_weights[:, 3:4] * integrated_pred
        )
...
class HierarchicalMultitaskModel(nn.Module):
    ...
    def __init__(self, num_features=44, base_dim=512, num_classes=6, 
        ...
        feature_dims = {
            'level1': self.hierarchical_encoder.level1_dim,
            'level2': self.hierarchical_encoder.level2_dim,
            'level3': self.hierarchical_encoder.level3_dim,
    ...
    def forward(self, features, input_ids=None, attention_mask=None, labels=None):
        ...
        hierarchical_features = self.hierarchical_encoder(features)
        
        # Get hierarchical classification predictions
        classification_outputs = self.hierarchical_classifier(hierarchical_features)
        
        # Use the final hierarchical prediction as the main classification output
        class_logits = classification_outputs['final']
        
        # Text generation with feature injection (if text inputs provided)
        ...
        return (
            ...
            generation_loss,                 # 2  
            generation_logits,               # 3
            self.classification_weight,      # 4
            self.generation_weight           # 5
        )
