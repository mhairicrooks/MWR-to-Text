...
class MultitaskModelWithProjection(nn.Module):
    """
    Multitask model with configurable feature projection strategies and dynamic loss weighting
    ...
    Args:
        ...
        dynamic_loss_weighting: bool, whether to use dynamic loss weighting based on performance
    """
    def __init__(self, num_features=44, embedding_dim=512, num_classes=6, 
                 ...
                 dynamic_loss_weighting=False, initial_class_weight=0.1,
                 initial_gen_weight=10.0):
        super().__init__()
        
        self.projection_mode = projection_mode
        self.use_minimal_text = use_minimal_text
        self.dynamic_loss_weighting = dynamic_loss_weighting
        
        # Dynamic loss weighting parameters
        if dynamic_loss_weighting:
            # Track recent losses for dynamic weighting
            self.class_loss_history = []
            self.gen_loss_history = []
            self.loss_history_size = 10  # Keep last N losses for averaging
            self.rebalance_frequency = 5  # Rebalance every N steps
            self.step_count = 0
            self.min_weight = 0.01
            self.max_weight = 20.0
            
        # Initialize loss weights (will be updated if dynamic_loss_weighting=True)
        self.classification_weight = initial_class_weight
        self.generation_weight = initial_gen_weight
    ...
    def _update_dynamic_weights(self, class_loss, gen_loss):
        """
        Update loss weights dynamically based on recent performance
        Uses inverse loss magnitude to balance tasks
        """
        if not self.dynamic_loss_weighting:
            return
            
        # Add losses to history
        self.class_loss_history.append(class_loss.item())
        self.gen_loss_history.append(gen_loss.item())
        
        # Keep only recent history
        if len(self.class_loss_history) > self.loss_history_size:
            self.class_loss_history.pop(0)
        if len(self.gen_loss_history) > self.loss_history_size:
            self.gen_loss_history.pop(0)
        
        self.step_count += 1
        
        # Rebalance weights periodically
        if self.step_count % self.rebalance_frequency == 0 and len(self.class_loss_history) >= 3:
            # Calculate average recent losses
            avg_class_loss = sum(self.class_loss_history[-3:]) / 3
            avg_gen_loss = sum(self.gen_loss_history[-3:]) / 3
            
            # Prevent division by zero
            avg_class_loss = max(avg_class_loss, 1e-8)
            avg_gen_loss = max(avg_gen_loss, 1e-8)
            
            # Inverse weighting: higher weight for task with lower loss (performing better)
            # This encourages balanced learning
            total_loss = avg_class_loss + avg_gen_loss
            
            # Update weights with smoothing factor
            smoothing = 0.1  # How much to update (0.1 = 10% adjustment)
            
            new_class_weight = (avg_gen_loss / total_loss) 
            new_gen_weight = (avg_class_loss / total_loss) 
            
            # Apply smoothing and bounds
            self.classification_weight = (1 - smoothing) * self.classification_weight + smoothing * new_class_weight
            self.generation_weight = (1 - smoothing) * self.generation_weight + smoothing * new_gen_weight
            
            # Clamp weights to reasonable bounds
            self.classification_weight = torch.clamp(torch.tensor(self.classification_weight), 
                                                   self.min_weight, self.max_weight).item()
            self.generation_weight = torch.clamp(torch.tensor(self.generation_weight), 
                                               self.min_weight, self.max_weight).item()
    ...
    def forward(self, features, input_ids=None, attention_mask=None, labels=None):
        ...
        if input_ids is not None and labels is not None:
            ...
            if self.dynamic_loss_weighting and self.training:
                # Calculate individual losses for weight updating
                class_loss = F.cross_entropy(class_logits, torch.argmax(labels[:, :class_logits.size(1)], dim=1) if labels.dim() > 1 else labels)
                gen_loss = t5_output.loss
                self._update_dynamic_weights(class_loss, gen_loss)
...
def create_experiment_pipeline(
    ...
    projection_mode='concat',  # Changed default to 'concat'
    projection_type='linear', 
    use_minimal_text=True,    # Changed default to True
    dynamic_loss_weighting=True,  # Added parameter
    ...
    """
    Set up a complete experiment pipeline for different projection modes with dynamic loss weighting
    """
    print(f"\n{'='*80}")
    ...
    print(f"Dynamic loss weighting: {'ENABLED' if dynamic_loss_weighting else 'DISABLED'}")
    print(f"Initial weights - Classification: {initial_class_weight}, Generation: {initial_gen_weight}")
    print(f"{'='*80}")
    ...
    # Initialize model with dynamic loss weighting
    model = MultitaskModelWithProjection(
        ...
        dynamic_loss_weighting=dynamic_loss_weighting,
        initial_class_weight=initial_class_weight,
        initial_gen_weight=initial_gen_weight
    )
    ...
    print(f"  Dynamic loss weighting: {dynamic_loss_weighting}")
