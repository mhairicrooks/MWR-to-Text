...
class AdapterModule(nn.Module):
    """
    Lightweight adapter module that can be inserted into transformer layers
    """
    def __init__(self, hidden_dim, adapter_dim=64, dropout=0.1):
        ...
        # Down-project to smaller dimension
        ...
        # Initialize to near-identity (but not exactly zero)
    ...
    def forward(self, x):
        ...
        return x + adapter_output  # Residual connection
...
class MultitaskModelWithAdapters(nn.Module):
    """
    Multitask model with configurable feature projection and adapter modules
    ...
    Args:
        ...
        use_adapters: bool, whether to add adapter modules to T5 layers
    ...
    def __init__(self, num_features=44, embedding_dim=512, num_classes=6, 
                 ...
                 use_adapters=False, adapter_dim=64, adapter_dropout=0.1,
                 freeze_t5_weights=False):
        super().__init__()
        
        self.projection_mode = projection_mode
        self.use_minimal_text = use_minimal_text
        self.use_adapters = use_adapters
        self.freeze_t5_weights = freeze_t5_weights
        ...
        # Store original forward methods before freezing
        self.original_encoder_forward = {}
        self.original_decoder_forward = {}
        
        # Add adapter modules to T5 layers BEFORE freezing weights
        if use_adapters:
            self.adapters = self._add_adapters_to_t5(adapter_dim, adapter_dropout)
        
        # Freeze T5 weights AFTER adding adapters
        if freeze_t5_weights:
    ...
    def _add_adapters_to_t5(self, adapter_dim, adapter_dropout):
        """
        Add adapter modules to T5 encoder and decoder layers using a more robust approach
        """
        t5_hidden_dim = self.t5.config.d_model
        adapters = nn.ModuleDict()

        # Add adapters to encoder layers
        for i, layer in enumerate(self.t5.encoder.block):
            ...
            self.original_encoder_forward[i] = original_forward

            def create_wrapped_forward(orig_forward, adapter_module):
                def wrapped_forward(hidden_states, attention_mask=None, position_bias=None,
                    ...
                    if isinstance(outputs, dict) or hasattr(outputs, "last_hidden_state"):
                        outputs.last_hidden_state = adapter_module(outputs.last_hidden_state)
                    else:
                        # For tuple output, apply adapter to first element (hidden states)
                        outputs = (adapter_module(outputs[0]),) + outputs[1:]


                    return outputs
                return wrapped_forward

            layer.forward = create_wrapped_forward(original_forward, adapter)

        # Add adapters to decoder layers
        for i, layer in enumerate(self.t5.decoder.block):
            adapter_name = f'decoder_layer_{i}'
            adapter = AdapterModule(
                hidden_dim=t5_hidden_dim,
                adapter_dim=adapter_dim,
                dropout=adapter_dropout
            ...
            original_forward = layer.forward
            self.original_decoder_forward[i] = original_forward
    ...
    def get_trainable_parameters(self):
        """
        Return count of trainable parameters (useful when using frozen T5 + adapters)
        """
...
def create_experiment_pipeline(
    ...
    use_adapters=True,  # New parameter for adapters
    adapter_dim=64,     # Adapter bottleneck dimension
    adapter_dropout=0.1,
    freeze_t5_weights=True,  # Usually freeze when using adapters
    ...
    """
    Set up experiment pipeline with adapter modules
    """
    print(f"\n{'='*80}")
    ...
    print(f"Adapters: {'ENABLED' if use_adapters else 'DISABLED'}")
    if use_adapters:
        print(f"Adapter dim: {adapter_dim}, dropout: {adapter_dropout}")
        print(f"T5 weights: {'FROZEN' if freeze_t5_weights else 'TRAINABLE'}")
    print(f"{'='*80}")
    
    # Load tokenizer
    tokenizer = T5Tokenizer.from_pretrained(tokenizer_path, local_files_only=True)
    
    # Create datasets (assuming ProjectionExperimentDataset is defined elsewhere)
    ...
    model = MultitaskModelWithAdapters(
        num_classes=2, 
        ...
        freeze_t5_weights=freeze_t5_weights
    )
